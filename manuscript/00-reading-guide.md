...
すぐ業務で使いたい場合は、[Artifact Index](../artifact-index.md) から必要な成果物を選び、該当章へ戻る。たとえば、依頼の品質が低いならRequest Contract、情報の前提が崩れるならContext Pack、レビューが弱いならAI出力レビュー表から入る。

### 3. 問題から読む

AI活用が詰まっている場合は、[Troubleshooting](../troubleshooting.md) を使って切り分ける。問題を「プロンプトが悪い」と即断せず、Work、Request、Context、Delegation、Control、Human / Orgのどこに原因があるかを確認する。
...
各章では、これらを必要な粒度で扱う。詳細なケース一覧は、[付録D: 職種別ユースケース集](../appendices/d-use-cases.md) を参照する。
...
章ごとの成果物は [Artifact Index](../artifact-index.md)、演習は [Exercise Bank](../exercises/exercise-bank.md) を参照する。
